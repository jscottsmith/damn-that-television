        ...
        function(TweenPlugin, TweenLite, Ease, VelocityTracker) {
            var ThrowPropsPlugin = function(props, priority) {
                ...
                _getClosest = function(n, values, max, min) {
                    var i = values.length, closest = 0, absDif = _max, val, dif;
                    while (--i > -1) {
                        val = values[i];
                        dif = val - n;
                        if (dif < 0) {
                ...
                _parseEnd = function(curProp, end, max, min) {
                    ...
                    var adjustedEnd = typeof curProp.end === 'function'
                        ? curProp.end(end)
                        : curProp.end instanceof Array
                              ? _getClosest(end, curProp.end, max, min)
                              : Number(curProp.end);
                    if (adjustedEnd > max) {
                        adjustedEnd = max;
                ...
                _calculateChange = (ThrowPropsPlugin.calculateChange = function(
                    ...
                    var e = ease instanceof Ease
                        ? ease
                        : !ease ? TweenLite.defaultEase : new Ease(ease);
                    return (
                        duration *
                        checkpoint *
                        velocity /
                        e.getRatio(checkpoint)
                    );
                }),
                ...
                _calculateTweenDuration = (ThrowPropsPlugin.calculateTweenDuration = function(
                    ...
                    var duration = 0,
                        ...
                        ease = vars.ease instanceof Ease
                            ? vars.ease
                            : !vars.ease
                        ...
                        if (
                            ...
                            if (typeof curProp !== 'object') {
                                ...
                                if (tracker && tracker.isTrackingProp(p)) {
                                    ...
                                    curDuration = curVelocity * resistance > 0
                            ...
                            if (typeof curProp === 'object') {
                                ...
                                curDuration = curVelocity * curResistance > 0
                                    ? curVelocity / curResistance
                                    : curVelocity / -curResistance;
                                curVal = typeof target[p] === 'function'
                                    ? target[
                                          p.indexOf('set') ||
                                              typeof target[
                                                  'get' + p.substr(3)
                ...
                p = (ThrowPropsPlugin.prototype = new TweenPlugin(
                    'throwProps'
                )),
                _cssProxy,
                _cssVars,
                            ...
                            if (
                                ...
                                if (typeof val === 'object') {
                                    if (
                                        ...
                                        velocities[p] =
                                            Number(val.velocity) || 0;
                                    } else {
                                        tracker =
                                            tracker ||
                                            VelocityTracker.getByTarget(t);
                                        velocities[p] = tracker &&
                                            tracker.isTrackingProp(p)
                                            ? tracker.getVelocity(p)
            ...
            p._onInitTween = function(target, value, tween) {
                ...
                var ease = tween._ease,
                    checkpoint = isNaN(value.checkpoint)
                        ? 0.05
                        : Number(value.checkpoint),
                ...
                for (p in value) {
                    if (
                        ...
                        if (typeof curProp === 'number') {
                            ...
                            typeof curProp === 'object' &&
                            !isNaN(curProp.velocity)
                        ) {
                            velocity = Number(curProp.velocity);
                        } else {
                            tracker =
                                tracker || VelocityTracker.getByTarget(target);
                            if (tracker && tracker.isTrackingProp(p)) {
            ...
            p.setRatio = function(v) {
                var i = this._props.length, cp, val;
                while (--i > -1) {
                    cp = this._props[i];
                    ...
                    if (cp.r) {
                        ...
                        val =
                            ((val * 10000 + (val < 0 ? -0.5 : 0.5)) | 0) /
                            10000; // if we don't round things at the very end, binary math issues can creep in and cause snapping not to be exact (like landing on 20.000000000001 instead of 20).
                    }
                    if (cp.f) {
    ...
    _gsScope._gsDefine(
        ...
        function(TweenLite) {
            var _first,
                ...
                _getComputedStyle = document.defaultView
                ...
                _update = function() {
                    var vt = _first, t = _ticker.time, val, vp;
            ...
            p.addProp = function(prop, type) {
                if (!this._lookup[prop]) {
                    ...
                    vp.css =
                        this.elem &&
                        (this.target.style[vp.p] !== undefined ||
                            _transforms[vp.p]);
                    ...
                    vp.type = type || (vp.css && prop.indexOf('rotation') === 0)
                        ? 'deg'
                        : '';
                    vp.v1 = vp.v2 = vp.css
                        ? _getStyle(t, vp.p)
                        : isFunc ? t[vp.p]() : t[vp.p];
                }
            };
            ...
            VelocityTracker.isTracking = function(target, prop) {
                ...
                return !vt
                    ? false
                    : !prop && vt._firstVP ? true : vt.isTrackingProp(prop);
            };
