...
module.exports = function loader(content) {
    ...
    if (!mime) {
        return loaderCallback(
            new Error(
                'No mime type for file with extension ' + ext + 'supported'
            )
        );
    }
    ...
    if (config.pass) {
        ...
        const f = loaderUtils.interpolateName(loaderContext, '[hash].[ext]', {
    ...
    const createFile = function createFile(_ref) {
        let data = _ref.data, width = _ref.width, height = _ref.height;

        const fileName = loaderUtils
            .interpolateName(loaderContext, name + ext, {
                context: outputContext,
                content: data,
            })
        ...
        return {
            src: '__webpack_public_path__ + ' +
                JSON.stringify(fileName + ' ' + width + 'w'),
    ...
    return img
        ...
        .then(function(_ref3) {
            let files = _ref3.files, placeholder = _ref3.placeholder;
